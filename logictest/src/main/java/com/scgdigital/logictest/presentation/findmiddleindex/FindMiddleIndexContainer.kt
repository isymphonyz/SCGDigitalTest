...
@Composable
fun FindMiddleIndexContainer(
    isSessionTimeout: Boolean,
    data: String?,
    onNavigate: (NavigationEvent) -> Unit,
) {

    val viewModel: FindMiddleIndexViewModel = getViewModel {
        parametersOf(isSessionTimeout)
    }

    val state = viewModel.state.observeAsState(FindMiddleIndexViewState()).value
    ...
    DisposableEffect(Unit) {
        viewModel.event.observe(lifecycleOwner) { event ->
            when (event) {
                is FindMiddleIndexViewEvent.Navigate -> onNavigate(event)
                is FindMiddleIndexViewEvent.ShowSnackbar -> coroutineScope.launch {
                    scaffoldState.snackbarHostState.showSnackbar(context.getString(event.message))
                }
                is FindMiddleIndexViewEvent.OpenBottomSheet -> {
